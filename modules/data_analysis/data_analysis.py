...
def create_model_analysis_dirs(glxy_path):
    plot_dir = glxy_path + 'test_plots/'
    csv_dir = glxy_path + 'csv_logs/'
...
def plot_galaxy_structure(structure, glxy_path, filename, title=None, label=None):
    ...
    out_plot, _ = create_model_analysis_dirs(glxy_path)
  
    plt.savefig(out_plot + filename)
    
    
def plot_velocity_component(distance, component, glxy_path, filename, title=None, label=None):
...
def galaxy_rotation_curve(structure_distances, structure_velocities, glxy_path, filename, title=None, labels=None):
    x_label = "Distance from galaxy center of mass [kpc]"
    y_label = "Velocity [km/s]"
