...
class EndMsg(Widget):
    """Displays a message at the end of the process."""
    
    __slots__ = ('format_string',)
    
    def __init__(self, format=' Done'):
        self.format_string = format
...
class Bar(WidgetHFill):
    ...
    __slots__ = ('marker', 'tip', 'left', 'right', 'fill', 'fill_left')

    def __init__(self, marker='#', tip='', left='|', right='|', fill=' ',
                 fill_left=True):
        """Creates a customizable progress bar.
        marker - string or updatable object to use as a marker
        tip - string or updatable object to use for the tip of the full part of the progress bar
        ...
        #If the tip is unspecified, then the tip of the bar is equal to the marker
        if tip == '':
            self.tip = self.marker
        else:
            self.tip = tip
    ...
    def update(self, pbar, width):
        ...
        left, marked, tip, right = (format_updatable(i, pbar) for i in
                               (self.left, self.marker, self.tip, self.right))

        width -= len(left) + len(right)
        # Marked must *always* have length of 1
        if pbar.maxval is not UnknownLength and pbar.maxval:
            marked *= int(pbar.currval / pbar.maxval * width)
            #Removes the last character of filled bar and adds the tip
            if pbar.currval < pbar.maxval:
                marked = marked[:-1]
                marked += tip
        else:
            marked = ''
