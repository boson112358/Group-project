...
class ProgressBar(object):
    ...
    __slots__ = ('currval', 'fd', 'finished', 'last_update_time',
                 ...
                 'term_width', 'update_interval', 'widgets', 'busy', 'thread', '_time_sensitive',
                 '_lock', '_increase_maxval', '__iterable')

    _DEFAULT_MAXVAL = 100
    _DEFAULT_TERMSIZE = 80
    _DEFAULT_WIDGETS = [widgets.Percentage(), ' ', widgets.Bar()]
    _DEFAULT_WITH_STATEMENT_MAXVAL = 0
    ...
    #with statement suport
    def __enter__(self):
        self._lock = threading.Lock()
        self.busy = True
        self.start_with()
        self.thread = threading.Thread(target=self.update_with)
        self.thread.start()
    ...
    def update_with(self):
        """With statement wrapper for update method"""
            
        while self.busy:
            with self._lock:
                if self._increase_maxval:
                    self.maxval += 1
                if self.update_interval == 0:
                    self.update_interval += 1
                
                time.sleep(1/(self.update_interval*10))
                self.update(self.currval + 1)
                
                
    def start_with(self):
        """With statement wrapper for start method"""
        
        if self.maxval is None:
            self.maxval = self._DEFAULT_WITH_STATEMENT_MAXVAL
            self._increase_maxval = True
    ...
    def update(self, value=None):
        ...
        if value is not None and value is not widgets.UnknownLength:
            if (self.maxval is not widgets.UnknownLength
                ...
                #if not self.busy or self.finished:
                raise ValueError('Value out of range, value: {}, max: {}'.format(value, self.maxval))
    ...
    def start(self):
        ...
        if self.maxval is not widgets.UnknownLength:
            if self.maxval < 0: raise ValueError('Value out of range, maxvalue: {}'.format(self.maxvalue))
